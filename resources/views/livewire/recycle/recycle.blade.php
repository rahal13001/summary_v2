        ...
        <div class="col-md-3 col-sm-3 d-flex">
            <div class="d-grid btn-group" role="group">
              ...
              <ul class="dropdown-menu">
                  <li><a class="dropdown-item" href="#" onclick="confirm('Yakin Ingin Menghapus {{ count($checked) }} Data ?')||event.stopImmediatePropagation()" wire:click="deleteDatas()">Hapus Permanen ({{ count($checked) }})</a></li>            
                  ...
                  <li><a class="dropdown-item" href="#" onclick="confirm('Yakin Ingin Mengembalikan {{ count($checked) }} Data ?')||event.stopImmediatePropagation()" wire:click="restoreDatas()">Pulihkan ({{ count($checked) }})</a></li>            
              </ul>
              @endif
            </div>
          </div>
              ...
              <tbody>
                @foreach ($datas as $data)
                  <tr class="@if ($this->isChecked($data->id)) table-primary @endif">
                    ...
                    <button class="btn btn-outline-success" wire:click ="restoreSatuData({{ $data->id }})"
                        onclick="confirm('Apakah Yakin Ingin Mengembalikan Data Laporan Berjudul {{ $data->what }} Tanggal {{ $data->when }} Milik {{ $data->user->name }}  ?')||event.stopImmediatePropagation()"><i class="bi bi-folder-symlink-fill"></i></button>
                      
                      @if (!$checked)
                      <a class="btn btn-outline-danger"
                      onclick="confirm('Apakah Yakin Ingin Menghapus Permanen Data Laporan Berjudul {{ $data->what }} Tanggal {{ $data->when }} Milik {{ $data->user->name }}  ?')||event.stopImmediatePropagation()"
