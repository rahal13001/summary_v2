...
class Allchartiku extends Component
{
    ...
    public function render()
    {
        ...
        $allIkuChart = (new ColumnChartModel())
                            ->setTitle('Jumlah');
                            // ->withOnColumnClickEventName('onColumnClick');
                    
        foreach ($this->indicators as $indexiku => $indicator) {
            $color = $colors[$indexiku % count($colors)]; // untuk setup warna
            $allIkuChart->addColumn($indicator->nomor, $indicator->report_count, $color);
        }



        return view('livewire.user.chart.allchartiku', compact('allIkuChart'));
    }
}
